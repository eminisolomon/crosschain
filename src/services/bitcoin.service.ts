...
import { _apiFallbacks } from '../utils/btc';
import { fallback, retryNTimes } from '../utils/retry';
import { GetTransactionPayload, TransferPayload } from '../interfaces';
import { BitgoUTXOLib } from '../libs/bitgo';
...
const transfer = async (data: TransferPayload) => {
  ...
  const keyPair = ECPair.fromWIF(data.privateKey);
  ...
  const amount = new BigNumber(data.amount.toString());
